...
**关键词：北航，北京航空航天大学，编译，编译技术，BUAA，BUAA-CP，评测机，测试用例，SysY，MIPS，期末，期末考试，竞速排序，2025秋**

一个用于测试 SysY 编译器的自动化测试框架，支持多线程并行测试、GUI 界面、多语言编译器（Java/C/C++）。

修改了课程组的Mars以减少运行时间和内存占用率，增加了并行任务数量和JVM参数，在开发者本机上实测2084个样例运行时间为2m15s。

如果电脑吃不消可以尝试调小并行任务数量，一般可以调整为略高于CPU核心数。
